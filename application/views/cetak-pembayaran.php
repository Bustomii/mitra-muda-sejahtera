...
foreach ($data->result_array() as $d) {
	...
	$nama 			= $d['namaanggota'];
	$tgl_pinjam 	= $this->app_model->tgl_str($d['tgl']);
	$lama		 	= $d['lama'];
	$bunga		 	= $d['bunga'];
...
$judul_H = "BUKTI PEMBAYARAN ANGGOTA<br>";
...
foreach ($data->result_array() as $r_data) {
	$tgl = $this->app_model->tgl_str($r_data['tgl_jatuh_tempo']);
	$tgl_bayar = $this->app_model->tgl_str($r_data['tgl_bayar']);
	$total_bayar = $r_data['jumlah_bayar'];
	$btotal += $r_data['jumlah_bayar'];
...
echo "<div class='akhir' align='right'>
			Total Bayar : <b>" . number_format($btotal) . "</b>
		</div>";
echo "<div class='akhir' align='right'>
			Total Angsuran : <b>" . number_format($gtotal) . "</b>
		</div>";
echo "<div class='akhir' align='right'>
			Sisa Angsuran : <b>" . number_format($gtotal - $btotal) . "</b>
		</div>";
